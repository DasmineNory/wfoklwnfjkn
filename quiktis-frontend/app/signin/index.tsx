...
const LoginPage: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await sendRequest({
        url: `${process.env.NEXT_PUBLIC_API_BASE_URL}/v1/users/login`,
        method: "POST",
        body: { email, password },
      });
  ...
  return (
    <div className="flex flex-col md:flex-row gap-8 justify-center text-white overflow-hidden mt-5 items-center w-full">
      {/* Form Section */}
      <div className="flex-1 flex flex-col justify-center p-6 w-full md:w-[505px]">
        ...
        <form className="w-full max-w-md space-y-6" onSubmit={handleSubmit}>
          ...
          />
          <Button type="submit" className="w-full py-4 mt-4 bg-primary">
            Login
          </Button>
        </form>
      ...
      {/* Image Section */}
      <div className="flex justify-center w-full md:w-[707px]">
        <Image
          src="/authImage.png"
          alt="auth"
          className="object-cover bg-cover rounded-[20px] w-full h-auto"
          width={707}
          height={950}
        />
      </div>
